...
class Photo: NSManagedObject {
    ...
    func doesItExist() {
        let path = pathForIdentifier(actualFileName)
        print("the poster path is \(path) ")
        let filemgr = NSFileManager.defaultManager()
        if filemgr.fileExistsAtPath(path) {
            print("-- **Executing doesItExist and File exists")
        } else {
            print("-- **Executing doesItExist and File not found")
        }
    ...
    func deleteImage() {
        let path = pathForIdentifier(actualFileName)
        print("the poster path to delete is \(path) ")
